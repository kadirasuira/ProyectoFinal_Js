...
// Obtén la fecha y hora actual con Luxon
function formatoTiempo() {
    const nuevoTiempo = luxon.DateTime.local();
    const formatoHora = nuevoTiempo.toFormat('h');
    const formatoMinutos = nuevoTiempo.toFormat('mm');
    const ampm = nuevoTiempo.toFormat('a');
    const dia = nuevoTiempo.toFormat('EEEE', { locale: 'es' }); // 'cccc' devuelve el nombre completo del día
        ...
        switch (contador) {
            case 0:
                ...
                case 1:
                    ...
                    if(opcion1 == 1){
                        Toastify({
                            text: "Iniciando el Juego...",
                            duration: 4000, // Duración en milisegundos
                            gravity: "bottom", // Posición de la tostada: "top", "bottom", "left", "right"
                            position: 'right', // Posición horizontal de la tostada
                          }).showToast();
...
// Función para cargar los datos desde un archivo JSON local
async function cargarDatosDesdeArchivo() {
    try {
        const response = await fetch('datos.json'); // Ajusta la ruta según sea necesario
        const datos = await response.json();
        palabras = datos.palabras;
        frases = datos.frases;
    } catch (error) {
        throw error; // Re-lanzar el error para que el consumidor de la función pueda manejarlo
    }
}
...
function jugarAhorcado(){
    ...
    .catch((error) => {
        ...
        Toastify({
            text: "Error Cargando Palabras..",
            duration: 4000, // Duración en milisegundos
            gravity: "bottom", // Posición de la tostada: "top", "bottom", "left", "right"
            position: 'right', // Posición horizontal de la tostada
          }).showToast(); // Borra todo el contenido dentro del elemento 'chatHistory'
    });
